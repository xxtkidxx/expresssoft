<%@ Control Language="C#" AutoEventWireup="true" CodeFile="MAVUNGQTDT.ascx.cs" Inherits="module_MAVUNGQTDT" %>
<telerik:RadCodeBlock ID="RadCodeBlockMAVUNGQTDT" runat="server">
    <script type="text/javascript">
        function RowDblClick(sender, eventArgs) {
            var CanEdit = "<%=ITCLIB.Security.Security.CanEditModule("BANGCUOC") %>";
        if ((eventArgs.get_tableView().get_name() == "MasterTableViewMAVUNGQTDT") && (CanEdit == "True")) {
    ...
    <script type="text/javascript">
        ...
        function cmbDoiTacClientSelectedIndexChangedHandler(sender, eventArgs) {
            $find("<%=RadGridMAVUNGQTDT.ClientID %>").get_masterTableView().rebind();
            return false;
        }
    </script>
...
<telerik:RadAjaxLoadingPanel Skin="Vista" ID="RadAjaxLoadingPanelMAVUNGQTDT" runat="server" />
<telerik:RadGrid ID="RadGridMAVUNGQTDT" runat="server" Skin="Vista"
    AllowPaging="True" PageSize="20" AllowSorting="True"
    AllowFilteringByColumn="True" GridLines="None" ShowStatusBar="True"
    AutoGenerateColumns="False"
    AllowMultiRowEdit="True" AllowAutomaticDeletes="True"
    AllowAutomaticInserts="True" AllowAutomaticUpdates="True"
    DataSourceID="MAVUNGQTDTDataSource" ShowFooter="True"
    OnDataBound="RadGridMAVUNGQTDT_DataBound"
    OnItemDeleted="RadGridMAVUNGQTDT_ItemDeleted" OnItemInserted="RadGridMAVUNGQTDT_ItemInserted"
    OnItemUpdated="RadGridMAVUNGQTDT_ItemUpdated"
    OnItemCommand="RadGridMAVUNGQTDT_ItemCommand"
    OnItemDataBound="RadGridMAVUNGQTDT_ItemDataBound" CellSpacing="0">
    ...
    <MasterTableView Name="MasterTableViewMAVUNGQTDT" CommandItemDisplay="Top" DataSourceID="MAVUNGQTDTDataSource" DataKeyNames="PK_ID" ClientDataKeyNames="PK_ID" EditMode="PopUp" NoMasterRecordsText="Không có dữ liệu">
        <CommandItemTemplate>
            <div style="padding: 5px 5px; float: left; width: auto">
                <b>Quản lý vùng tính cước</b>&nbsp;&nbsp;&nbsp;&nbsp;
                        <asp:LinkButton ID="btnEditSelected" runat="server" CommandName="EditSelected" Visible='<%# RadGridMAVUNGQTDT.EditIndexes.Count == 0 && ITCLIB.Security.Security.CanEditModule("BANGCUOC") %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/Edit.gif" />Sửa</asp:LinkButton>&nbsp;&nbsp;
                        <asp:LinkButton ID="btnUpdateEdited" runat="server" CommandName="UpdateEdited" Visible='<%# RadGridMAVUNGQTDT.EditIndexes.Count > 0 %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/Update.gif" />Lưu</asp:LinkButton>&nbsp;&nbsp;
                        <asp:LinkButton ID="btnCancel" runat="server" CommandName="CancelAll" Visible='<%# RadGridMAVUNGQTDT.EditIndexes.Count > 0 || RadGridMAVUNGQTDT.MasterTableView.IsItemInserted %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/Cancel.gif" />Hủy bỏ</asp:LinkButton>&nbsp;&nbsp;
                        <asp:LinkButton ID="LinkButton2" runat="server" CommandName="InitInsert" Visible='<%# !RadGridMAVUNGQTDT.MasterTableView.IsItemInserted && ITCLIB.Security.Security.CanAddModule("BANGCUOC") %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/AddRecord.gif" />Thêm</asp:LinkButton>&nbsp;&nbsp;
                        <asp:LinkButton ID="LinkButton3" runat="server" CommandName="PerformInsert" Visible='<%# RadGridMAVUNGQTDT.MasterTableView.IsItemInserted %>'><img style="border:0px;vertical-align:middle;" alt="" src="Images/Grid/Insert.gif" />Lưu</asp:LinkButton>&nbsp;&nbsp;
...
<asp:SqlDataSource ID="MAVUNGQTDTDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ExpressSoftConnectionString %>"
    DeleteCommand="DELETE FROM [DMMAVUNGDT] WHERE [PK_ID] = @PK_ID"
    InsertCommand="INSERT INTO [DMMAVUNGDT] ([FK_MASANPHAM], [FK_DOITAC], [C_CODE], [C_NAME], [C_DESC],[C_TYPE],[FK_VUNGLAMVIEC]) VALUES (@FK_MASANPHAM,@FK_DOITAC,@C_CODE, @C_NAME, @C_DESC,2,@FK_VUNGLAMVIEC)"
    SelectCommand="SELECT [DMMAVUNGDT].[PK_ID], [DMMAVUNGDT].[FK_MASANPHAM], [DMMAVUNGDT].[FK_DOITAC], [DMMAVUNGDT].[C_CODE], [DMMAVUNGDT].[C_NAME], [DMMAVUNGDT].[C_DESC],DMMASANPHAM.C_NAME as MASANPHAMNAME FROM [DMMAVUNGDT] LEFT OUTER JOIN DMMASANPHAM ON DMMAVUNGDT.FK_MASANPHAM = DMMASANPHAM.PK_ID WHERE DMMAVUNGDT.C_TYPE = 2 AND DMMAVUNGDT.FK_VUNGLAMVIEC = @FK_VUNGLAMVIEC AND (@FK_DOITAC = 0 OR [DMMAVUNGDT].[FK_DOITAC] = @FK_DOITAC) ORDER BY DMMAVUNGDT.PK_ID"
...
<asp:SqlDataSource ID="MASANPHAMDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:ExpressSoftConnectionString %>"
    SelectCommand="SELECT [PK_ID], [C_CODE], [C_NAME] FROM [DMMASANPHAM]  WHERE C_TYPE = N'Quốc tế' ORDER BY PK_ID"></asp:SqlDataSource>
<asp:SqlDataSource ID="SqlDataSourceKHUVUC" runat="server" ConnectionString="<%$ ConnectionStrings:ExpressSoftConnectionString %>"
    SelectCommand="SELECT [PK_ID], [C_CODE], [C_NAME] FROM [DMKHUVUC]"></asp:SqlDataSource>
